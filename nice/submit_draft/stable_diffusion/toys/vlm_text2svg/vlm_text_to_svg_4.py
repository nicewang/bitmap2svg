...
MAX_SVG_SIZE_BYTES = 10000
SVG_SIZE_SAFETY_MARGIN = 1000
...
def generate_image(prompt: str, negative_prompt: str, **kwargs) -> Image.Image:
    ...
    with torch.no_grad():
        result = pipe(prompt=prompt, negative_prompt=negative_prompt, **default_kwargs)

    torch.cuda.empty_cache()

    return result.images[0]

def compress_hex_color(r: int, g: int, b: int) -> str:
...
def perform_color_quantization(image: np.ndarray, num_colors_hint: int = 0) -> Tuple[np.ndarray, List[Tuple[int, int, int]]]:
    """
...
def extract_contours_for_color(quantized_image: np.ndarray, target_color: Tuple[int, int, int], 
...
def calculate_feature_importance(contour: np.ndarray, area: float, 
...
def classify_segments_enhanced(image: Image.Image, quantized_image: np.ndarray, 
    ...
    for color in palette:
        ...
        predicted_category = semantic_categories[top_idx.item()]
...
def get_category_style_enhanced(category: str) -> Dict[str, str]:
...
def create_enhanced_svg(image: Image.Image, quantized_image: np.ndarray, 
    ...
    for segment in segments_info:
        ...
        contours = extract_contours_for_color(quantized_image, color)
    ...
    bg_color = compress_hex_color(avg_color[0], avg_color[1], avg_color[2])
    ...
    for feature in all_features:
        ...
        if current_size > (MAX_SVG_SIZE_BYTES - SVG_SIZE_SAFETY_MARGIN):
        ...
        style_from_category = get_category_style_enhanced(category)

        # 2. Get the REAL color from the K-Means quantization process for this feature.
        real_color_hex = compress_hex_color(*feature['color'])
...
def text_to_svg_enhanced( 
    ...
    image = generate_image(prompt=prompt, negative_prompt=negative_prompt, width=width, height=height, **generation_kwargs)

    # Step 2: Enhanced color quantization with full adaptive logic
    img_array = np.array(image)
    quantized_image, palette = perform_color_quantization(img_array, num_colors)

    # Step 3: Classify segments
    segments_info = classify_segments_enhanced(image, quantized_image, palette)

    # Step 4: Create enhanced SVG using the corrected logic
    svg_content = create_enhanced_svg(
...
if __name__ == "__main__":
    svg_content, image = text_to_svg_enhanced(
