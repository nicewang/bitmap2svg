...
// You might need to adjust the include path based on your Potrace installation
#include <potracelib.h>


// Helper function to create potrace_bitmap_t from raw pixel data for a specific color mask.
// This is a simplified approach for demonstration. Proper color quantization and masking
// would be more robust for complex color images.
...
const char* convert_image_to_svg(int width, int height, const uint8_t* pixels, int channels) {
    ...
    if (channels == 1) {
        ...
        // Sort unique colors for consistent layering (e.g., lighter colors first)
        // Or sort by frequency/importance, but simple color sort is easier here.
        // std::vector<std::vector<uint8_t>> sorted_colors(unique_colors.begin(), unique_colors.end());
        // std::sort(sorted_colors.begin(), sorted_colors.end(), [](const auto& a, const auto& b){
        //    // Simple sort by sum of RGB values
